...
- 协议默认 `rate_limit=1000ms`、`pre_delay`/`post_delay=200ms`——省略字段会引入隐式等待，
  不需要延迟时要显式写 `0`。
- 轮数由玩家意愿决定的流程（爬塔这类「打到打不动为止」的），界应该是**时间**而不是次数：
  用 `agent/recognition/timer.py` 的 `TimerExpired` 闸门（与 `counter.py` 的次数计数互补），
  别去凑 `max_hit`。闸门的识别逻辑是反的——已超时才算命中，所以挂在循环节点 `next` 首位
  不会改变原有候选的顺序语义。
