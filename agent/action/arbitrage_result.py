...
class ArbitrageSellController(CustomAction):
    def run(self, context: Context, argv: CustomAction.RunArg) -> bool:
        ...
        for idx, target in enumerate(targets_to_sell, 1):
            ...
            sell_result = False
            sell_detail = None                 # 循环可能在首轮 run_task 前 break(停止指令),下游按未启动处理
            for att, cand in enumerate(cands):
                ...
                # 起点设为进入出售菜单的识别节点
                # run_task 返回 Optional[TaskDetail]:返回对象只表示任务被成功提交,成败在 .status——
                # 旧写法把返回值当 bool 恒真。sell_result 供候选循环的断链守卫沿用同一判读。
                sell_detail = context.run_task("Arbitrage_Sell_HUB", pipeline_override=override_cfg)
                sell_result = sell_detail is not None and sell_detail.status.succeeded
