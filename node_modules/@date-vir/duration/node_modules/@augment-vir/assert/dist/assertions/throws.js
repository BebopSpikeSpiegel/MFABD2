...
function internalDoesNotThrowCheck(checkType, callbackOrPromise, failureMessage) {
    try {
        const result = callbackOrPromise instanceof Promise ? callbackOrPromise : callbackOrPromise();
        if (result instanceof Promise) {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    if (checkType === ThrowsCheckType.CheckWrap) {
                        ...
                        reject(new AssertionError(`Expected no error but got '${extractErrorMessage(caughtError)}'.`, failureMessage));
                    }
                }
            });
        }
        ...
        if (checkType === ThrowsCheckType.CheckWrap) {
            ...
            throw new AssertionError(`Expected no error but got '${extractErrorMessage(caughtError)}'.`, failureMessage);
        }
    }
}
...
function doesNotThrow(callbackOrPromise, failureMessage) {
    return internalDoesNotThrowCheck(ThrowsCheckType.Assert, callbackOrPromise, failureMessage);
}
function doesNotThrowCheck(callbackOrPromise) {
    return internalDoesNotThrowCheck(ThrowsCheckType.Check, callbackOrPromise);
}
function doesNotThrowAssertWrap(callbackOrPromise, failureMessage) {
    return internalDoesNotThrowCheck(ThrowsCheckType.AssertWrap, callbackOrPromise, failureMessage);
}
function doesNotThrowCheckWrap(callbackOrPromise, failureMessage) {
    return internalDoesNotThrowCheck(ThrowsCheckType.CheckWrap, callbackOrPromise, failureMessage);
}
...
export const throwGuards = {
    ...
    waitUntil: {
         ...
         * @throws {@link AssertionError} On timeout.
         */
        doesNotThrow: internalWaitUntilDoesNotThrow,
        /**
