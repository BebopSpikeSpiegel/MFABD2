...
var Bundle = class extends EventEmitter {
	...
	constructor(loader, watcher, maa, root, parser) {
		super();
		this.maa = maa;
		this.root = root;
		this.parser = parser;
...
function extractTaskRef(r) {
	if (r.type === "task.anchor" || r.type === "task.reco" || r.type === "task.color_filter" || r.type === "task.custom_task" || r.type === "task.entry") return r.target;
...
function isAnchorRef(r) {
	return (r.type === "task.next" || r.type === "task.roi" || r.type === "task.target" || r.type === "task.custom_anchor") && !!r.attrs.attrs.Anchor;
}
...
async function buildDiagnosticMessage(root, diag, evalPos, _option) {
	...
	const buildBrief = async () => {
		switch (diag.type) {
			...
			case "int-conflict-group": return t("maa.pipeline.error.conflict-group", diag.group, await buildPos(diag.previous));
			case "int-unknown-group": return t("maa.pipeline.error.unknown-group", diag.group);
...
//#endregion
//#region src/parser/interface/group.ts
...
var InterfaceBundle = class extends EventEmitter {
	...
	constructor(loader, watcher, maa, root, file = "interface.json", parser) {
		super();
		this.maa = maa;
		this.root = root;
	...
	updatePaths() {
		...
		if (finalPaths.length > 0) {
			...
			this.bundles = this.paths.map((dir) => {
				return new Bundle(this.content.loader, this.content.watcher, this.maa, path.join(this.root, dir), this.parser);
			});
		} else {
			for (const bundle of this.bundles) bundle.stop();
			this.paths = [];
